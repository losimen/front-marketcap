<template>
  <div class="grid grid-cols-[repeat(10,auto)] bg-white rounded-[24px] w-full border-[1px] border-[#D8973C] max-[900px]:shadow-[2px_2px_0px_0px_#D8973C] shadow-[3px_4px_0px_0px_#D8973C]">
    ...
    :class="isLastIndex(index) ? 'border-t-[1px]' : 'border-y-[1px]'">
      ...
      <div>
        <div class="w-full flex justify-end px-2">
          <StatsPercentageChange
            :value="parseFloat(numberWithCommas(coin.variation_1h ?? 0))"
            class="py-[22px]"
          />
        </div>
      </div>
      <div>
        <div class="w-full flex justify-end px-2">
          <StatsPercentageChange :value="parseFloat(numberWithCommas(coin.variation_6h ?? 0))" class="py-[22px]" />
        </div>
      </div>
      <div>
        <div class="w-full flex justify-end px-2">
          <StatsPercentageChange :value="parseFloat(numberWithCommas(coin.variation_24h ?? 0))" class="py-[22px]" />
        </div>
      </div>
      <div>
        <div class="w-full flex justify-end px-2">
            <span class="font-inter leading-[20px] font-[500] text-[14px] max-[900px]:text-[12px] text-[#1A1C1F] text-end py-[22px]">
              ${{ parseFloat(numberWithCommas(coin.market_cap ?? 0)) }}
            </span>
        </div>
      </div>
      <div>
        <div class="w-full flex justify-end px-2">
          <div class="font-inter leading-[20px] font-[500] text-[14px] max-[900px]:text-[12px] text-[#1A1C1F] text-end py-[14px]">
            <div>${{ parseFloat(numberWithCommas(coin.total_volume ?? 0)) }}</div>
            <div class="flex">
                <span class="text-[#AFAFAF] mr-1">
                  {{ numberWithCommas(parseFloat(((coin.total_volume ?? 0) / (coin.price ?? 1)).toFixed(0))) }}
                </span>
...
import TableCoinChart from './Charts/TableCoinChart.vue'
